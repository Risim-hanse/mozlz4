...
{
    switch (rc) {
    case MOZLZ4_ERR_NULL:       return "Internal error (NULL)";
    case MOZLZ4_ERR_MAGIC:      return "Unrecognized input file";
    case MOZLZ4_ERR_TOO_SHORT:  return "Input too short";
    case MOZLZ4_ERR_DECOMPRESS: return "Malformed input file";
    case MOZLZ4_ERR_COMPRESS:   return "Compression failed";
    case MOZLZ4_ERR_TOO_LARGE:  return "Input too large";
...
int main(int argc, char **argv)
{
    ...
    if (!do_compress) {
        ...
        rc = mozlz4_read_size(input_data, input_len, &decompressed_size);
        if (rc != MOZLZ4_OK) {
        ...
        /* sanity cap for crafted headers */
        if (decompressed_size > MOZLZ4_HEADER_LEN + (1ULL << 30)) {
        ...
        size_t out_cap = (size_t)decompressed_size;
        ...
        if (bound == 0) {
            fprintf(stderr, "Error: input too large to compress\n");
            free(input_data);
            return 1;
        }
